-- =============================================================================
-- Pricing Engine Auditor + Checkout Calculation Guard (gc_commerce migrated data)
--
-- Run after backfill + integrity views. Use:
--   SELECT * FROM gc_commerce.v_pricing_checkout_audit_summary;
-- Detail views return only failing / risky rows (empty = clean).
--
-- Definitions:
--   - "Pricing inconsistency": line money fields disagree with each other or legacy source.
--   - "Calculation mismatch": order header does not reconcile to lines + declared tax/shipping.
--   - "Margin risk": per-unit revenue below per-unit cost (where cost exists on public.products),
--     or missing cost (analytics gap, not necessarily wrong pricing).
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- B) Migrated unit price vs legacy order_items (source of truth for backfill)
-- -----------------------------------------------------------------------------
...
SELECT
  ...
  ol.unit_price_minor AS gc_unit_price_minor,
  ROUND(COALESCE(oi.unit_price, 0) * 100)::BIGINT AS legacy_unit_price_minor,
  (ol.unit_price_minor - ROUND(COALESCE(oi.unit_price, 0) * 100)::BIGINT) AS delta_unit_minor,
  ol.line_subtotal_minor AS gc_line_subtotal_minor,
  ROUND((COALESCE(oi.unit_price, 0) * oi.quantity::NUMERIC) * 100)::BIGINT AS legacy_extended_minor,
  (ol.line_subtotal_minor - ROUND((COALESCE(oi.unit_price, 0) * oi.quantity::NUMERIC) * 100)::BIGINT) AS delta_extended_minor,
  'gc_line_vs_legacy_order_item' AS issue_category
FROM gc_commerce.order_lines ol
INNER JOIN public.order_items oi ON oi.id = (NULLIF(ol.product_snapshot->>'legacy_order_item_id', ''))::BIGINT
WHERE ABS(ol.unit_price_minor - ROUND(COALESCE(oi.unit_price, 0) * 100)::BIGINT) > 1
   OR ABS(
     ol.line_subtotal_minor
     - ROUND((COALESCE(oi.unit_price, 0) * oi.quantity::NUMERIC) * 100)::BIGINT
   ) > 1;

COMMENT ON VIEW gc_commerce.v_audit_line_vs_legacy_unit_price IS
  'Migrated lines whose cents do not match public.order_items unit_price * qty (backfill drift).';

-- -----------------------------------------------------------------------------
-- C) Checkout guard: header equation (same as v_integrity_gc_header_equation_failures)
--    Exposed under audit namespace for one-stop reporting.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- E) Checkout guard: sum(line total_minor) + shipping + tax - discount vs order total
--    (When lines carry full line totals incl. tax/discount, this should match header.)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- F) Tax allocation signal: header tax > 0 but all lines tax_minor = 0
--    (Informational / risk: not a pricing error if business rules keep tax on header only.)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- H) Order-level margin rollup (where every line has known cost)
-- -----------------------------------------------------------------------------
