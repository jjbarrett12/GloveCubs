 ...
 * - If SKU exists in DB: update product (only overwrite when incoming value non-empty; preserve image_url if blank). Count as "updated".
 * - If SKU does not exist: create new product. Count as "created".
 * - Empty/short rows: count as "skipped". Invalid rows: count as "failed", collect errorSamples (max 20).
 * @returns {{ created, updated, skipped, failed, deleted, parsedRows, withImage, skusInImport, errorSamples }}
 */
function upsertProductsFromCsv(db, csvContent, options = {}) {
    ...
    if (lines.length < 2) {
        return { created: 0, updated: 0, skipped: 0, failed: 0, deleted: 0, parsedRows: 0, withImage: 0, skusInImport: new Set(), errorSamples: [] };
    }
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (values.length < headerCount) {
            skipped++;
            if (errorSamples.length < MAX_ERROR_SAMPLES) errorSamples.push({ row: lineNum, message: 'Too few columns' });
            continue;
        }
        ...
        if (!skuLower) {
            failed++;
            if (errorSamples.length < MAX_ERROR_SAMPLES) errorSamples.push({ row: lineNum, message: 'Missing SKU' });
            continue;
        }
        ...
        if (!data) {
            failed++;
            if (errorSamples.length < MAX_ERROR_SAMPLES) errorSamples.push({ row: lineNum, sku: skuLower || undefined, message: 'Missing required: name, brand, material, or price' });
            continue;
        }
        ...
        if (existing) {
            ...
            Object.keys(data).forEach((k) => {
                const v = data[k];
                if (v !== undefined && v !== null && (typeof v !== 'string' || v.trim() !== '')) toApply[k] = v;
            });
            if (!(toApply.image_url || '').trim() && (existing.image_url || '').trim()) toApply.image_url = existing.image_url;
            Object.assign(existing, toApply);
            updated++;
        } else {
    ...
    console.log('[CSV Import] parsedRows:', parsedRows, '| created:', created, '| updated:', updated, '| skipped:', skipped, '| failed:', failed);

    return { created, updated, skipped, failed, deleted, parsedRows, dataRowCount: parsedRows, withImage, skusInImport, errorSamples };
}
