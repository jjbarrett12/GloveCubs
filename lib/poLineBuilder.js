 ...
 * Source of truth per line: catalog variant UUID (canonical_product_id) +
 * active supplier_offers.sku (manufacturer-facing) for the supplier linked to public.manufacturers.
 */
 ...
 * @param {{ orderId?: string|number }} ctx
 * @returns {Promise<
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    if (!canonical) {
      ...
      blocked_lines.push({
        ...
        message: 'Order line is missing canonical_product_id (catalogos.products UUID).',
      });
      continue;
    }
    ...
    if (!pr) {
      ...
      blocked_lines.push({
        ...
        message: `No catalogos.products row for id=${canonical}.`,
      });
      continue;
    }
    ...
    const poLine = {
      ...
      supplier_offer_id: offer.id,
      sku: supplierSku,
      name: (item.name || item.product_name || offer.product_name || '').trim() || supplierSku,
