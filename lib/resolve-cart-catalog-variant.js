...
/**
 * Resolves catalog_v2.catalog_variants for commerce lines.
 *
 * Phase 0D: size-only and single-variant inference paths are deprecated (Phase 0B hard enforcement).
 * Do not add new importers; see scripts/check-variant-mandatory.js.
 */

const { normalizeCanonicalUuidInput, resolveLineCatalogProductId } = require('./resolve-canonical-product-id');
const { isVariantMandatoryEnforceEnabled } = require('./commercial-line-identity');
const { warnVariantInference } = require('./commerce-truth-warnings');
...
async function resolveCatalogVariantForCommerceLine(supabase, line) {
  ...
  if (isVariantMandatoryEnforceEnabled()) {
    return {
      ...
      message:
        'catalog_variant_id and variant_sku are required (VARIANT_MANDATORY_ENFORCE). Size-only and auto-pick paths are disabled.',
    };
  }
  ...
  if (sizeRaw) {
    ...
    warnVariantInference({
      module: 'lib/resolve-cart-catalog-variant.js',
      path: 'resolveCatalogVariantForCommerceLine',
      reason: 'size_code_match',
      catalog_product_id: parent,
      size: sizeRaw,
  ...
  if (all.length === 1) {
    ...
    warnVariantInference({
      module: 'lib/resolve-cart-catalog-variant.js',
      path: 'resolveCatalogVariantForCommerceLine',
      reason: 'single_active_variant_auto_pick',
      catalog_product_id: parent,
