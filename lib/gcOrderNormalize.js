...
function normalizeGcOrderLinesForApi(lines, sellableById) {
  ...
  return (lines || []).map((ol) => {
    ...
    const snapSellableSku = snap.sellable_sku != null ? String(snap.sellable_sku) : '';
    const variantSkuSnap = snap.variant_sku != null ? String(snap.variant_sku) : null;
    return {
      order_line_id: ol.id,
      ...
      sku: variantSkuSnap || sp.sku || '',
      sellable_sku: snapSellableSku || sp.sku || '',
      catalog_variant_id: ol.catalog_variant_id != null ? String(ol.catalog_variant_id) : null,
      variant_sku: variantSkuSnap,
    };
  });
}
