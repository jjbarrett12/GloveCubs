...
/**
 * Checkout/cart line unit pricing (cart display, order rows, PaymentIntent).
 *
 * Phase 0D: compatibility layer — deprecated in favor of Pricing Authority V2 (Phase 0A/0E).
 * Do not add new importers; see scripts/check-pricing-drift.js.
 *
...
const { warnPricingDeprecated } = require('./commerce-truth-warnings');
...
function resolveLineUnitPriceForCheckout({
  ...
  warnPricingDeprecated({
    module: 'lib/commerce-pricing.js',
    path: 'resolveLineUnitPriceForCheckout',
    reason: 'sellable_products + margin/tier; replace with resolve_pricing_authority_v2',
    catalog_product_id: product?.id != null ? String(product.id) : undefined,
  });
