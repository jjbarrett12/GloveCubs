...
function ssrfResultToResponse(result, originalUrl) {
    if (!result.ok) {
        const err = new Error(result.error || 'SSRF-safe fetch failed');
        err.security_blocked = !!result.security_blocked;
        throw err;
    }
    ...
    return {
        ...
        url: result.final_url || originalUrl,
        headers: {
            get(name) {
                const n = String(name).toLowerCase();
                if (n === 'content-type') return result.content_type || '';
                if (n === 'content-length') return String(buf.length);
                return null;
            }
        },
        async text() {
...
async function fetchWithTimeout(url, options = {}, timeoutMs = PARSE_URL_TIMEOUT_MS) {
    ...
    return ssrfResultToResponse(result, url);
}
...
/**
 * Extract spec table / features section text and bullet list items for attribute inference.
 * Returns { specText: string, bullets: string[] }.
 */
function extractSpecTextAndBullets(html) {
    const specText = [];
    const bullets = [];
    if (!html || typeof html !== 'string') return { specText: '', bullets: [] };
    const slice = html.slice(0, 100000);
    const sectionHeads = /<(?:h[2-6]|div|section|th)[^>]*>[\s\S]*?(?:Specifications?|Features?|Product\s+Details?|Details?|Technical|Specs?)[\s\S]*?<\/\w+>/gi;
    let m;
    while ((m = sectionHeads.exec(slice)) !== null) {
        const block = slice.slice(m.index, Math.min(m.index + 8000, slice.length));
        const stripped = block.replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
        if (stripped.length > 20) specText.push(stripped);
    }
    const tableRows = slice.match(/<tr[^>]*>[\s\S]*?<\/tr>/gi) || [];
    for (const row of tableRows) {
        const cellText = row.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
        if (cellText.length > 5 && cellText.length < 500) specText.push(cellText);
    }
    const ulLi = slice.match(/<li[^>]*>([\s\S]*?)<\/li>/gi) || [];
    for (const li of ulLi) {
        const inner = (li.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim()).slice(0, 300);
        if (inner.length > 2) bullets.push(inner);
    }
    return {
        specText: specText.join(' ').slice(0, 15000),
        bullets: bullets.slice(0, 80)
    };
}

/**
 * Extract JSON-LD blocks and meta for admin AI flow. Returns { jsonld, meta, text, specText, bullets }.
 */
function extractForAdmin(html, finalUrl) {
    const jsonld = [];
    const meta = { title: '', image: '', description: '' };
    if (!html || typeof html !== 'string') return { jsonld, meta, text: '', specText: '', bullets: [] };
    ...
    const { specText, bullets } = extractSpecTextAndBullets(html);
    return { jsonld, meta, text: extractMainText(html), specText, bullets };
}
...
function classifyProbeResult(probe, url) {
    ...
    if (isHtmlContentType(contentType) && body) {
        ...
        const { jsonld, meta, text, specText, bullets } = extractForAdmin(body, finalUrl || u);
        return {
            kind: 'page',
            url: u,
            finalUrl: finalUrl || u,
    ...
    if (payload.kind === 'page' && payload.extracted) {
        ...
        if (hospeco && hospeco.isHospecoUrl(payload.finalUrl || u)) {
            try {
                ...
                if (enriched.productDetails && typeof enriched.productDetails === 'object') {
                    payload.extracted.productDetails = enriched.productDetails;
                }
            } catch (err) {
