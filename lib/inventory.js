...
const { warnParentInventory } = require('./commerce-truth-warnings');
const {
...
} = require('./variant-inventory-authority');

/**
 * Parent-grain inventory (public.inventory.canonical_product_id).
 * Phase 0D: deprecated for checkout reservation — Variant Inventory Authority (Phase 0C).
 * Do not add new reserve/check paths; see scripts/check-parent-inventory-usage.js.
 */
...
async function reserveStockForOrder(orderId, items, userId = null) {
  ...
  if (isVariantInventoryAuthorityEnabled()) {
    for (const item of items) {
      const q0 = Number(item.quantity) || 0;
      if (q0 <= 0) continue;
      const vid = normalizeCanonicalUuidInput(item.catalog_variant_id);
      if (!vid) {
        throw new Error('catalog_variant_id is required for variant inventory reservation');
      }
      const c0 = normalizeCanonicalUuidInput(item.canonical_product_id);
      if (c0) await assertCatalogV2ProductIdForCommerce(c0, 'inventory_reserve_variant');
    }

    const variantAvail = await checkVariantAvailability(items, { failClosed: true });
    if (!variantAvail.ok) {
      const first = variantAvail.insufficient[0];
      const idLabel = first.catalog_variant_id || first.canonical_product_id || '(missing variant id)';
      throw new Error(
        first.code === 'MISSING_VARIANT_INVENTORY'
          ? `Missing variant_inventory row for ${idLabel}`
          : `Insufficient variant stock for ${idLabel}: need ${first.needed}, available ${first.available ?? 0}`,
      );
    }

    if (!(await _isOrderReserved(orderId))) {
      const pItems = mapItemsForVariantReserveRpc(items);
      ...
      const { data, error } = await supabase.rpc('gc_reserve_variant_stock_for_order_atomic', {
        p_order_id: String(orderId),
        p_user_id: uidGc,
        p_items: pItems,
      });
      if (error) {
        throw new Error(error.message || 'Variant reservation failed');
      }
      if (!(data && data.skipped === true)) {
        console.log(`[inventory] Reserved variant stock for gc order ${orderId}`);
      }
    } else {
  ...
  warnParentInventory({
    ...
    path: 'reserveStockForOrder',
    reason: 'gc_reserve_stock_for_order_atomic uses parent canonical_product_id',
    order_id: orderId != null ? String(orderId) : undefined,
    line_count: Array.isArray(items) ? items.length : 0,
  });
...
async function releaseStockForOrder(orderId, userId = null) {
  ...
  if (isVariantInventoryAuthorityEnabled()) {
    ...
    const uidGc = userId != null && isAuthUserUuid(userId) ? String(userId) : null;
    const { data, error } = await supabase.rpc('gc_release_variant_stock_for_order_atomic', {
      p_order_id: String(orderId),
      p_user_id: uidGc,
    });
    if (error) {
      throw new Error(error.message || 'Variant release failed');
    }
    if (!(data && data.skipped === true)) {
      console.log(`[inventory] Released variant stock for gc order ${orderId}`);
    }
    return;
  }
...
async function deductStockForOrder(orderId, userId = null) {
  ...
  if (isVariantInventoryAuthorityEnabled()) {
    ...
    const uidGc = userId != null && isAuthUserUuid(userId) ? String(userId) : null;
    const { data, error } = await supabase.rpc('gc_deduct_variant_stock_for_order_atomic', {
      p_order_id: String(orderId),
      p_user_id: uidGc,
    });
    if (error) {
      throw new Error(error.message || 'Variant deduct failed');
    }
    if (!(data && data.skipped === true)) {
      console.log(`[inventory] Deducted variant stock for gc order ${orderId}`);
    }
    return;
  }
...
async function adjustStock(canonicalProductId, delta, reason, reference, userId = null) {
  const { assertLegacyWarehouseWriteBlocked } = require('./legacy-warehouse-deprecation');
...
async function checkAvailability(items) {
  if (isVariantInventoryAuthorityEnabled()) {
    for (const item of items || []) {
      const c0 = normalizeCanonicalUuidInput(resolveLineCatalogProductId(item));
      if (c0) await assertCatalogV2ProductIdForCommerce(c0, 'inventory_check_availability_variant');
    }
    const result = await checkVariantAvailability(items, { failClosed: true });
    if (isVariantInventoryShadowEnabled()) {
      for (const item of items || []) {
        const canon = normalizeCanonicalUuidInput(resolveLineCatalogProductId(item));
        const parentStock = canon ? await getStockForLineItem({ canonical_product_id: canon }) : null;
        setImmediate(() => {
          shadowCompareVariantInventoryLine({
            item,
            parentAvailable: parentStock ? parentStock.available_stock : null,
            flow: 'checkout_check_availability',
          }).catch(() => {});
        });
      }
    }
  ...
  warnParentInventory({
    ...
    reason: 'parent canonical_product_id stock check before checkout',
    line_count: Array.isArray(items) ? items.length : 0,
  });
  ...
  for (const item of items) {
    ...
    if (isVariantInventoryShadowEnabled()) {
      setImmediate(() => {
        shadowCompareVariantInventoryLine({
          item,
          parentAvailable: stock ? stock.available_stock : null,
          flow: 'checkout_check_availability',
        }).catch(() => {});
      });
    }
...
/**
 * Delete parent-grain inventory row for a catalog product (admin product purge).
 * @param {string} canonicalProductId catalog_v2.catalog_products.id
 * @returns {Promise<{ ok: true } | { error: string }>}
 */
async function deleteInventoryForCanonicalProduct(canonicalProductId) {
  const canonical = normalizeCanonicalUuidInput(canonicalProductId);
  if (!canonical) {
    return { error: 'canonical_product_id required for inventory delete' };
  }
  const supabase = getSupabaseAdmin();
  const { error } = await supabase.from('inventory').delete().eq('canonical_product_id', canonical);
