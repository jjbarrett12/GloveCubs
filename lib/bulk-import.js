...
async function approveDraft(supabase, draftId) {
    ...
    if (brand) {
        ...
        if (existingMfr?.id) manufacturer_id = existingMfr.id;
        else {
            const { data: inserted, error: ins } = await supabase.from('manufacturers').insert({ name: brand }).select('id').single();
            if (!ins && inserted) manufacturer_id = inserted.id;
        }
    }
    ...
    let productId;
    if (existingProduct) {
        const { error: upErr } = await supabase.from('products').update(productPayload).eq('id', existingProduct.id);
        if (upErr) throw upErr;
        productId = existingProduct.id;
    } else {
        productPayload.created_at = new Date().toISOString();
        const { data: inserted, error: inErr } = await supabase.from('products').insert(productPayload).select('id').single();
    ...
    if (draft.import_job_item_id) {
        await supabase
            .from('import_job_items')
            .update({ created_product_id: productId, updated_at: new Date().toISOString() })
            .eq('id', draft.import_job_item_id);
    }
    return { product_id: productId, action: existingProduct ? 'updated' : 'created' };
}
