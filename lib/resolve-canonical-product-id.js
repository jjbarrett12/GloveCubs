...
/**
 * V2 commerce: persisted cart/order lines use catalog_v2.catalog_products.id on
 * canonical_product_id + product_id; catalogos.products.id is carried as listing_id
 * for pricing and storefront fetches.
 */
...
class MissingCanonicalProductIdError extends Error {
  ...
  constructor(context) {
    super(`V2 commerce requires a resolvable catalogos listing UUID on every line (context=${context}).`);
...
/**
 * catalogos.products.id when present (for getProductById / pricing). Absent on legacy cart rows.
 * @param {{ listing_id?: unknown }} line
 * @returns {string | null}
 */
function resolveCartLineListingProductId(line) {
  return normalizeCanonicalUuidInput(line && line.listing_id);
}

/**
 * Mutate lines in place: resolve catalogos listing UUID → catalog_v2.catalog_products.id;
 * sets listing_id (catalogos), canonical_product_id + product_id (catalog_v2).
 * @param {Array<{ product_id?: unknown, canonical_product_id?: unknown, listing_id?: unknown, size?: unknown }>} lines
 * @param {string} context
 */
async function ensureCommerceLinesHaveCanonical(lines, context) {
  ...
  for (const line of list) {
    let catalogosSourceId = normalizeCanonicalUuidInput(line.listing_id);
    if (!catalogosSourceId) {
      const ca = normalizeCanonicalUuidInput(line.canonical_product_id);
      const pb = normalizeCanonicalUuidInput(line.product_id);
      if (ca && pb && ca !== pb) {
        throw new MissingCanonicalProductIdError(
          `${context}: product_id and canonical_product_id disagree; send listing_id or matching catalogos UUIDs`,
        );
      }
      catalogosSourceId = ca || pb;
    }
    if (!catalogosSourceId) {
    ...
    const v2Id = await resolveCatalogV2ProductId(catalogosSourceId);
    line.listing_id = catalogosSourceId;
    line.canonical_product_id = v2Id;
    line.product_id = v2Id;
    await assertCatalogV2ProductIdForCommerce(v2Id, context);
  }
  return list;
}
