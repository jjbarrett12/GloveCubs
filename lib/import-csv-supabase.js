...
async function importCsvToSupabase(csvContent, options = {}) {
    ...
    if (deleteNotInImport && result.skusInImport.size > 0) {
        const admin = getSupabaseAdmin();
        const { data: allProducts } = await admin.schema('catalogos').from('products').select('id, sku');
        const toDelete = (allProducts || []).filter((p) => !result.skusInImport.has((p.sku || '').toString().trim().toLowerCase()));
