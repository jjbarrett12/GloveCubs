...
function inferAttributesHeuristic(input) {
  ...
  // Thickness (mil): canonical parse so "12mil", "12-mil", "12 mil" → one value "12"; all thicknesses listed (no 7+)
  const milMatch = combined.match(/(\d+(?:\.\d+)?)\s*[-]?\s*(?:mil|mm)\b/i) || combined.match(/\b(\d+(?:\.\d+)?)\s*(?:mil|mm)?/i) || (pd.thickness && pd.thickness.match(/(\d+(?:\.\d+)?)/));
  if (milMatch) {
    const num = Math.round(parseFloat(milMatch[1] || milMatch[2] || milMatch[3] || milMatch[0]));
    if (Number.isFinite(num) && num >= 0) attrs.thickness_mil = [String(num)];
  }
