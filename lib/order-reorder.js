...
const { resolveCheckoutLineUnitPrice } = require('./pricing-authority-checkout');
const {
...
} = require('./commercial-line-identity');
  ...
  for (const item of orderItems || []) {
    ...
    if (isVariantMandatoryEnforceEnabled()) {
      if (!catalogVariantId || !variantSku) {
        status = 'unavailable';
        reason = 'MISSING_VARIANT_IDENTITY';
      } else {
        const idRes = await assertCommercialLineIdentity(supabase, {
        ...
        if (!idRes.ok) {
          status = 'unavailable';
          reason = idRes.code || idRes.message;
        } else {
          catalogVariantId = idRes.catalog_variant_id;
          variantSku = idRes.variant_sku;
        }
      }
    } else if (catalogVariantId && variantSku) {
      const idRes = await assertCommercialLineIdentity(supabase, {
        catalog_variant_id: catalogVariantId,
        variant_sku: variantSku,
        canonical_product_id: catalogId,
    ...
    if (product && status === 'available' && catalogVariantId && variantSku) {
      const priceRes = await resolveCheckoutLineUnitPrice({
        flow: 'reorder_preview',
        user,
        companyId,
        product,
      ...
      if (priceRes.ok) {
        currentUnitPrice = priceRes.unitPrice;
      } else {
        status = 'unavailable';
        reason = priceRes.code || priceRes.message;
      }
    }
...
function resolveReorderSelections(previews, requestedLines) {
  ...
  for (const r of requestedLines) {
    ...
    } else if (!isVariantMandatoryEnforceEnabled()) {
