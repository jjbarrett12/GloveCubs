 ...
 *
 * Canonical contract for the **Next storefront** lives in `storefront/src/lib/ai/schemas.ts`.
 * This CommonJS module remains the source of truth for **Express** `server.js` AI routes until those routes are retired.
 * Do not assume field parity between the two without an explicit migration — shapes diverged by design (snake_case vs camelCase, etc.).
 */
