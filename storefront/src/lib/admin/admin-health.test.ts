...
describe("admin-health", () => {
  ...
  it("marks Express-backed modules unavailable when JWT_SECRET is missing", () => {
    stubHealthySupabase();
    vi.stubEnv("NEXT_PUBLIC_GLOVECUBS_API", "http://localhost:3004");
    vi.stubEnv("JWT_SECRET", "");

    const health = resolveAdminHealth();
    for (const moduleId of EXPRESS_ADMIN_MODULE_IDS) {
      const availability = getAdminModuleAvailability(health, moduleId);
      expect(availability.available).toBe(false);
      expect(availability.status).toBe("setup_required");
    }
  });

  it("marks Express-backed modules unavailable when NEXT_PUBLIC_GLOVECUBS_API is missing", () => {
    stubHealthySupabase();
  ...
  it("treats production missing Express bridge as production_blocking", () => {
    stubHealthySupabase();
    ...
    expect(availability.available).toBe(false);
    expect(availability.status).toBe("production_blocking");
    expect(availability.reason).toBe("production_blocking");
  });
      ...
      issues: [
        {
          ...
          message: "Fulfillment modules cannot authenticate.",
          moduleIds: ["purchase-orders"],
        },
      ],
    });
