    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {chips.map((c) => (
        <Link
          key={c.href + c.label}
          href={c.href}
          className="inline-flex items-center gap-1 rounded-full border border-[#FF7A00]/40 bg-[#FF7A00]/10 px-2.5 py-1 text-xs font-medium text-[#ffb27a] hover:border-[#FF7A00]/60"
        >
          {c.label}
          <X className="h-3 w-3 opacity-80" aria-hidden />
        </Link>
      ))}
      <Link href={clearHref} className="text-[11px] font-semibold text-white/50 underline-offset-2 hover:text-[#FF7A00] hover:underline">
        Clear all
      </Link>
    </div>
  );
