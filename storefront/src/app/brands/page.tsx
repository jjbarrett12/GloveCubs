...
export default function BrandsPage() {
  return (
    ...
    >
      <div className="mb-8 rounded-lg border border-[#FF7A00]/30 bg-[#FF7A00]/10 px-4 py-3 text-sm text-white/85">
        ...
        {HOME_BRAND_LIST.map((name) => {
          ...
          return (
            <li key={name}>
              <Link
                href={href}
                className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 transition hover:border-[#FF7A00]/45 hover:bg-white/[0.07]"
              >
                {logo ? (
      ...
      <p className="mt-8 text-sm text-white/55">
        Missing a line card?{" "}
        <Link href="/request-pricing" className="font-medium text-[#FF7A00] hover:underline">
