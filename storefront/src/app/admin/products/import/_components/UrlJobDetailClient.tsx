...
function ImageEvidenceStrip({ images }: { images: string[] }) {
  if (images.length === 0) {
    return (
      <div className={cn(adminMutedPanel, "flex h-16 items-center justify-center border-solid px-3 text-xs text-admin-muted")}>
        No images extracted
      </div>
    );
  }
    ...
    <div className="flex flex-wrap gap-2">
      {images.slice(0, 6).map((url, i) => (
        <a
          ...
          className="block h-16 w-16 overflow-hidden rounded-lg border border-admin-border bg-admin-surface-muted"
          title={url}
        >
      ...
      {images.length > 6 ? (
        <span className="self-end font-mono text-[10px] text-admin-muted">+{images.length - 6} more</span>
      ) : null}
    </div>
  );
}
...
function DuplicateMatchList({ matches }: { matches: UrlImportExtractedProduct["duplicateCandidates"] }) {
  ...
  return (
    <div className={adminAlertSurface("warning", "text-sm")}>
      <p className="text-xs font-semibold uppercase tracking-wide">Duplicate candidates ({matches.length})</p>
      <ul className="mt-1 space-y-1">
        {matches.slice(0, 5).map((m, i) => (
          <li key={`${m.targetId ?? "_"}-${i}`}>
            ...
            {m.reasons.length > 0 ? (
              <span className="ml-2">· {m.reasons.slice(0, 3).join(" · ")}</span>
            ) : null}
          </li>
        ))}
      </ul>
...
function ExtractedProductCard({
  ...
  return (
    <article className={cn(adminCardSurface, "p-5")}>
      <div className="flex items-start gap-3">
        <input
          ...
          className="mt-1 h-4 w-4 rounded border-admin-border text-admin-accent focus:ring-admin-accent/30"
        />
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <h3 className="text-sm font-semibold text-admin-primary">{product.title}</h3>
            {product.confidence != null ? (
              <span className="font-mono text-xs text-admin-muted" title="Extraction confidence from import service">
                conf {(product.confidence * 100).toFixed(0)}%
                {product.aiUsed ? " · AI" : ""}
              </span>
            ) : null}
          </div>
          <dl className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-admin-secondary sm:grid-cols-3">
            {product.brand ? (
              <div className="flex gap-2">
                <dt className="text-admin-muted">Brand</dt>
                <dd className="text-admin-primary">{product.brand}</dd>
              </div>
            ) : null}
            {product.sku ? (
              <div className="flex gap-2">
                <dt className="text-admin-muted">SKU</dt>
                <dd className="font-mono text-admin-primary">{product.sku}</dd>
              </div>
            ) : null}
            {product.mpn ? (
              <div className="flex gap-2">
                <dt className="text-admin-muted">MPN</dt>
                <dd className="font-mono text-admin-primary">{product.mpn}</dd>
              </div>
            ) : null}
            {product.gtin ? (
              <div className="flex gap-2">
                <dt className="text-admin-muted">GTIN/UPC</dt>
                <dd className="font-mono text-admin-primary">{product.gtin}</dd>
              </div>
            ) : null}
            {product.size ? (
              <div className="flex gap-2">
                <dt className="text-admin-muted">Inferred size</dt>
                <dd className="text-admin-primary">{product.size}</dd>
              </div>
            ) : null}
            {product.baseSku ? (
              <div className="flex gap-2">
                <dt className="text-admin-muted">Base SKU</dt>
                <dd className="font-mono text-admin-primary">{product.baseSku}</dd>
              </div>
            ) : null}
          </dl>
          ...
          {product.attributes.length > 0 ? (
            <ul className="mt-3 flex flex-wrap gap-1.5">
              {product.attributes.map((a) => (
                <li
                  key={a.key}
                  className="rounded-md border border-admin-border bg-admin-surface-muted px-2 py-0.5 text-xs text-admin-secondary"
                >
                  <span className="text-admin-muted">{a.key}:</span> <span className="text-admin-primary">{a.value}</span>
                </li>
              ))}
            </ul>
          ) : null}
          ...
          {product.warnings.length > 0 ? (
            <div className={cn(adminAlertSurface("warning", "mt-3 text-sm"))}>
...
function BridgeActionBar({
  ...
  return (
    <div className="sticky bottom-3 z-10 rounded-xl border border-admin-border bg-admin-surface/95 px-4 py-4 shadow-lg backdrop-blur supports-[backdrop-filter]:bg-admin-surface/90">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3 text-sm text-admin-secondary">
          <span>
            <span className="font-mono font-semibold text-admin-primary">{selectedCount}</span> of{" "}
            <span className="font-mono font-semibold text-admin-primary">{totalCount}</span> extracted rows selected
          </span>
          <button
            type="button"
            onClick={onSelectAll}
        ...
        <div className="flex items-center gap-3">
          {disabledReason ? (
            <span className="text-xs font-medium text-admin-warning">{disabledReason}</span>
          ) : null}
          <button
            type="button"
            onClick={onBridge}
            disabled={disabled}
            className={cn(adminPrimaryButton, "disabled:cursor-not-allowed")}
  ...
  if (!detail) {
    return (
      <div className={adminAlertSurface("critical")}>
        ...
        <div className="mt-2">
          <button type="button" onClick={() => void refresh()} className={cn(adminSecondaryButton, "text-xs")}>
            Retry
          </button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 pb-24">
      <div className={cn(adminCardSurface, "p-5")}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <h2 className="truncate text-lg font-semibold text-admin-primary">{job.supplierName}</h2>
            </div>
            <p className="mt-1 truncate font-mono text-xs text-admin-muted">{job.startUrl || "—"}</p>
            <p className="mt-0.5 font-mono text-xs text-admin-muted">job id: {job.id}</p>
          </div>
        ...
        <dl className="mt-4 grid gap-2 text-sm text-admin-secondary sm:grid-cols-3">
          <div>
            <dt className="text-xs font-semibold text-admin-muted">Created</dt>
            <dd className="font-mono text-admin-primary">{formatDate(job.createdAt)}</dd>
          </div>
          <div>
            <dt className="text-xs font-semibold text-admin-muted">Started</dt>
            <dd className="font-mono text-admin-primary">{formatDate(job.startedAt)}</dd>
          </div>
          <div>
            <dt className="text-xs font-semibold text-admin-muted">Finished</dt>
            <dd className="font-mono text-admin-primary">{formatDate(job.finishedAt)}</dd>
          </div>
        </dl>

        {job.warnings.length > 0 ? (
          <div className={cn(adminAlertSurface("warning", "mt-4 text-sm"))}>
            <p className="text-xs font-semibold uppercase tracking-wide">Job warnings ({job.warnings.length})</p>
            <ul className="mt-1 list-inside list-disc space-y-0.5">
        ...
        {error ? (
          <div className={cn(adminAlertSurface("critical", "mt-4"))}>{error}</div>
        ) : null}
      </div>
