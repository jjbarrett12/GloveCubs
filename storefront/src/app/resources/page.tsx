    ...
    >
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        {CARDS.map((c) => (
          <div
            key={c.href}
            className="flex flex-col rounded-xl border border-white/10 bg-white/[0.04] p-6 transition hover:border-[#FF7A00]/40"
          >
            <h2 className="text-lg font-semibold text-white">{c.title}</h2>
            <p className="mt-2 flex-1 text-sm text-white/65">{c.body}</p>
            <Link
              href={c.href}
              className="mt-5 inline-flex w-fit rounded-lg bg-[#FF7A00] px-4 py-2 text-sm font-semibold text-white hover:bg-[#e56e00]"
            >
              {c.cta}
            </Link>
          </div>
