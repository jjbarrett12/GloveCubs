        ...
        {INDUSTRY_KEYS.map((key: IndustryKey) => {
          ...
          return (
            <Link
              key={key}
              href={`/industries/${key}`}
              className="rounded-xl border border-white/10 bg-white/[0.04] p-6 transition hover:border-[#FF7A00]/50 hover:bg-white/[0.07]"
            >
              <h2 className="text-xl font-semibold text-white">{cfg.name}</h2>
              <p className="mt-2 text-sm text-white/65">{cfg.tagline}</p>
              <span className="mt-4 inline-block text-sm font-semibold text-[#FF7A00]">View industry →</span>
            </Link>
          );
        })}
      </div>
